      ...
      <div className="grid gap-6 grid-cols-2 md:grid-cols-4">
        ...
        <div className="p-6 bg-white border border-slate-200 rounded-3xl shadow-sm space-y-2 flex flex-col justify-between">
          ...
          <div className="flex items-center gap-1.5">
            ...
            <Badge className="bg-emerald-50 text-emerald-700 border-emerald-200 text-[8px] font-bold px-1.5 py-0.5">
              100% 매뉴얼 준수
            </Badge>
          </div>
        </div>

        {/* Card 3: Saved hours (30 min per doc) */}
        <div className="p-6 bg-white border border-slate-200 rounded-3xl shadow-sm space-y-2 flex flex-col justify-between">
            ...
            <div key={event.id} className="p-6 bg-white border border-slate-200 rounded-3xl shadow-sm space-y-4 hover:shadow-md transition-shadow">
                ...
                <div className="space-y-1">
                  <div className="flex items-center gap-2">
                    <Badge className="bg-blue-50 text-blue-700 border-blue-200 font-bold">
                      초등 {event.grade}학년 {event.subject}
                    </Badge>
                  ...
                  <p className="text-xs text-slate-500 font-semibold">
                    체험처: {event.placeName} | 예정일: {event.tripDate}
                  </p>
                </div>
              ...
              <div className="flex flex-wrap gap-2 pt-2 border-t border-slate-100">
                <Link href={`/document/${event.id}`}>
                  <Button variant="outline" className="text-xs font-bold border-slate-350 rounded-xl px-4 py-3">
                    📄 계획서 조율
                  </Button>
                </Link>
                ...
                <Link href={`/previsit/${event.id}`}>
                  <Button className="bg-blue-600 hover:bg-blue-700 text-white text-xs font-bold rounded-xl px-4 py-3 shadow-sm">
                    📸 현장 사전답사 작성
                  </Button>
                </Link>
                <Link href={`/mobile-guide/${event.id}`}>
                  <Button className="bg-amber-500 hover:bg-amber-600 text-white text-xs font-bold rounded-xl px-4 py-3 shadow-sm">
                    📱 당일 모바일 가이드
                  </Button>
                </Link>
              </div>
